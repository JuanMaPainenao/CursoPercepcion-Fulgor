...
static rcl_publisher_t        s_pos_pub;    /* potenciómetro: posición 0..100 % */
static rcl_publisher_t        s_volt_pub;   /* potenciómetro: voltaje  0..3.3 V  */
static rcl_publisher_t        s_angle_pub;  /* AS5600: ángulo 0..360 °           */
static std_msgs__msg__Float32 s_pos_msg;
static std_msgs__msg__Float32 s_volt_msg;
static std_msgs__msg__Float32 s_angle_msg;
...
static void timer_callback(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    /* Tensión medida en el ADC (0..3.3 V). */
    float voltage = (raw * 3.3f) / 4095.0f;

    float position = (voltage - POT_VMIN_V) * 100.0f /
                     (POT_VMAX_V - POT_VMIN_V);
    ...
    s_volt_msg.data = voltage;

    /* ================= AS5600 ================= */

    uint16_t as_raw   = as5600_get_raw();     /* RAW ANGLE   (0..4095) */
    uint16_t as_angle = as5600_get_angle();   /* ANGLE       (0..4095) */

    s_angle_msg.data = as_raw * 360.0f / 4096.0f;   /* grados 0..360 */

    /* ================= PUBLICACIÓN ================= */

    RCSOFTCHECK(rcl_publish(&s_pos_pub,   &s_pos_msg,   NULL));
    RCSOFTCHECK(rcl_publish(&s_volt_pub,  &s_volt_msg,  NULL));
    RCSOFTCHECK(rcl_publish(&s_angle_pub, &s_angle_msg, NULL));

    ESP_LOGI(TAG,
             "POT raw=%4d pos=%.1f%% volt=%.2fV || AS5600 raw=%4u angle=%4u deg=%.1f",
             raw,
             s_pos_msg.data,
             s_volt_msg.data,
             as_raw,
...
static void micro_ros_task(void *arg)
{
    ...
    RCCHECK(rclc_node_init_default(&node, "esp32_sensores", "", &support));
    ESP_LOGI(TAG, "Nodo /esp32_sensores creado");
    ...
    /* Un solo handle: el timer. Los publicadores no cuentan como handles. */
    ...
    RCCHECK(rcl_publisher_fini(&s_angle_pub, &node));
